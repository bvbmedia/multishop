...
if (count($payment_methods)) {
	...
	if (count($shipping_methods)) {
		...
		foreach ($shipping_methods as $row) {
			...
			foreach ($payment_methods as $payment_method) {
				...
				if ($GLOBALS['TYPO3_DB']->sql_num_rows($qry2) > 0)	$content.='checked';
				$content.=' /></td>';
			}			
			$content.='</tr>';
		}
	...
	else $content.='Currently there isn\'t any shipping methods defined.';
}
else $content.='Currently there isn\'t any payment methods defined.';
