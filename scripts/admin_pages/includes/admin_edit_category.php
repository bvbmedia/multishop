	...
	if ($category['categories_id'] or $_REQUEST['action']=='add_category') {
		...
		if ($_REQUEST['action']=='add_category') {
			...
			if ($_REQUEST['action']=='edit_category') {
				...
				$heading_page='<div class="panel-heading"><h3>'.$this->pi_getLL('edit_category').' (ID: '.$category['categories_id'].')</h3><a class="btn btn-primary viewfront" href="'.$details_link.'" target="_blank"><i class="fa fa-eye"></i> '.$this->pi_getLL('admin_edit_view_front_category', 'View in front').'</a></div>';
			}
		}
		$category_name_block='';
