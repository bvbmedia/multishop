...
if ($this->post['proceed_order']) {
	...
	if ($customer_id) {
		...
		$url=$this->FULL_HTTP_URL.mslib_fe::typolink($this->shop_pid.',2003', '&tx_multishop_pi1[page_section]=admin_ajax&orders_id='.$orders_id.'&tx_multishop_pi1[is_manual]=1&action=edit_order&tx_multishop_pi1[is_proposal]='.$this->post['tx_multishop_pi1']['is_proposal'], 1);
		header('Location: '.$url);
		exit();
	} //add to orders eof
}
