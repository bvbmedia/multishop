...
if ($this->post) {
	...
	if ($product['products_id'] or $_REQUEST['action']=='add_product') {
		...
		if ($_REQUEST['action']=='add_product') {
			...
			$heading_page='<h3>'.$this->pi_getLL('admin_edit_product').' (ID: '.$product['products_id'].')</h3><a href="'.$details_link.'" target="_blank" class="btn btn-primary viewfront pull-right"><i class="fa fa-eye"></i> '.$this->pi_getLL('admin_edit_view_front_product', 'View in front').'</a>';
		}
		/*
		 * js header
		 */
