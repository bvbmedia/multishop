	...
	foreach ($categories_name as $category_name) {
		...
		if (!empty($category_name)) {
			$query3=$GLOBALS['TYPO3_DB']->SELECTquery('cd.categories_name, c.categories_id, c.parent_id', // SELECT ...
				...
				'c.page_uid='.$this->showCatalogFromPage.' and c.parent_id ='.addslashes($this->post['parent_id']).' and cd.categories_name=\''.addslashes($category_name).'\' and c.categories_id=cd.categories_id', // WHERE...
				'', // GROUP BY...
				'', // ORDER BY...
				'' // LIMIT ...
			);
...
<hr>
	...
	<form class="form-horizontal admin_add_multiple_categories blockSubmitForm" name="admin_add_multiple_categories" id="admin_add_multiple_categories" method="post" action="'.mslib_fe::typolink($this->shop_pid.',2003', '&tx_multishop_pi1[page_section]=add_multiple_category').'" enctype="multipart/form-data">
	<input type="hidden" name="tx_multishop_pi1[referrer]" id="msAdminReferrer" value="'.$subpartArray['###VALUE_REFERRER###'].'" >
	<input type="hidden" name="action" id="msAdminReferrer" value="add_multiple_category" >
	';
