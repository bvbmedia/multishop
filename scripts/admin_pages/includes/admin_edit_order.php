        ...
        jQuery(document).ready(function($) {
            ...
            if ($this->ms['MODULES']['ENABLE_EDIT_ORDER_PAYMENT_CONDITION_FIELD'] && $this->ms['MODULES']['ORDER_EDIT']) {
                if (!$orders['is_locked'] || ($orders['is_locked'] && $orders['payment_condition'] != '')) {
                    ...
                    if (!$orders['is_locked']) {
                        $orderDetailsItem .= '<div class="col-md-9"><div class="input-group width-fw"><input class="form-control" type="text" name="order_payment_condition" id="order_payment_condition" value="' . ($orders['payment_condition'] ? htmlspecialchars($orders['payment_condition']) : $this->ms['MODULES']['DEFAULT_PAYMENT_CONDITION_VALUE']) . '" /><span class="input-group-addon">' . $this->pi_getLL('days') . '</span></div></div>';
                    } else {
