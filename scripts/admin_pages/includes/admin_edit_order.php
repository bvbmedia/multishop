	...
	if ($orders['orders_id']) {
		...
		while (($row=$GLOBALS['TYPO3_DB']->sql_fetch_assoc($qry2)) != false) {
			...
			while (($row3=$GLOBALS['TYPO3_DB']->sql_fetch_assoc($qry3)) != false) {	
				if ($row3['price_prefix']=='+') $total_amount=$total_amount+($row['qty']*$row3['options_values_price']);
				else							$total_amount=$total_amount-($row['qty']*$row3['options_values_price']);
...
if ($this->ms['MODULES']['ORDER_EDIT'] and !$orders['is_locked'])
{
	...
	if (is_array($shipping_methods) and count($shipping_methods))
	{
		...
		foreach ($shipping_methods as $code => $item) {
			...
			if ($code==$orders['shipping_method']) $dontOverrideDefaultOption=1;
		}		
...
//		$tmpcontent.='<tr><td colspan="'.$colspan.'"><hr class="hr"></td></tr>';
	...
	if (!$orders['expected_delivery_date']) $orders['expected_delivery_date']=time();
	$tmpcontent.='
	</div>	
	<div class="account-field">
...
$GLOBALS['TSFE']->additionalHeaderData[] = '
	...
	if (count($order_status_history_items) > 0)
	{
		...
		foreach ($order_status_history_items as $row)
		{
			...
			if (!$old_status_name) $old_status_name='Unknown Order Status';
			$status_name=$all_orders_status[$row['new_value']]['name'];
			if (!$status_name) $status_name='Unknown Order Status';
	...
	$tabs['Order_Status']=array($this->pi_getLL('order_status'),$tmpcontent);
