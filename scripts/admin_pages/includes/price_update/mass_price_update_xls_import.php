...
function getOptionID($optname) {
	...
	if(empty($option_id) || $option_id == 0) return false;
...
function getOptionValueID($optval) {
	...
	if(empty($option_val_id) || $option_val_id == 0) return false;
	else return $option_val_id;
}
...
function getProductAttributeID($pid, $optid, $optvalid) {
	...
	if(empty($pa_id) || $pa_id == 0) return false;
	else return $pa_id;
}
			...
			switch($col_key) {
				...
				default :
					if($colname[$col_key] == 'capital price') {
						$cprice = $col_val;
					} else if($colname[$col_key] == 'dealer price') {
						$dprice = $col_val;
					} else if($colname[$col_key] == 'minimum order') {
						$morder = $col_val;
					} else if($colname[$col_key] == 'ean') {
						$eancode = $col_val;
					} else if($colname[$col_key] == 'relation') {
						$relation = $col_val;
					} else if($colname[$col_key] == 'status') {
						$status = $col_val;
					} else if($colname[$col_key] == 'neg. keywords') {
						$neg_keyword = $col_val;
					} else if($colname[$col_key] == 'promotext') {
						$promotext = $col_val;
					} else if($colname[$col_key] == 'leveranciers') {
						$leverancier = $col_val;
					} else if($colname[$col_key] == 'image url') {
						$image_url = $col_val;
					} else if($colname[$col_key] == 'extra attributes') {
						$extra_attributes = $col_val;
					} else if(substr($colname[$col_key], 0, 2) == 'a:') {
							...
							if(!empty($optval)) {
								...
								foreach($value_tmp as $optval2) {
									...
									if(substr($optvalprice, 0, 1) == '+') {
										...
										$pprefix = '+';
									} else if(substr($optvalprice, 0, 1) == '-') {
										$optvalprice = str_replace('-', '', $optvalprice);
										$pprefix = '-';
									} else {
										if($optvalprice != 0) {
									...
									} else if(strpos($optval3, ':unhide') !== FALSE) {
