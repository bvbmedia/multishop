	...
	//	if ($_REQUEST['q'] and strlen($_REQUEST['q']) < 3) exit();
		...
		if ($have_paging) {
			$tmp_listing .= '<li class="ui-menu-item" role="menuitem">
								<a alt="'.urlencode($this->get['q']).'" class="ui-corner-all" tabindex="-1" href="'.mslib_fe::typolink($this->shop_pid.',2003','tx_multishop_pi1[page_section]=admin_search&page='.($this->get['page'] + 1).'&ms_admin_skeyword='.urlencode($this->get['q'])).'">
									<span id="more-results">'.htmlspecialchars($this->pi_getLL('more_results')).' (Page '. ($this->get['page'] + 1) .' from '.$global_max_page.')</span>
								</a>
							</li>';
			...
			if ($results_counter > 0) {
				$tmp_listing .= '<li class="ui-menu-item" role="menuitem">
								<a alt="'.urlencode($this->get['q']).'" class="ui-corner-all" tabindex="-1">
									<span id="more-results">(Page '. ($this->get['page'] + 1) .' of '.$global_max_page.')</span>
								</a>
							</li>';
			} else {
				$tmp_listing .= '<li class="ui-menu-item" role="menuitem">
