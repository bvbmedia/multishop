...
if (($handle = opendir($this->DOCUMENT_ROOT.t3lib_extMgm::siteRelPath('multishop').'scripts/admin_pages/includes/admin_import_parser_templates')) != false) {
	while (false !== ($file = readdir($handle))) {
		if ($file != "." && $file != "..") {
			if ($file) $file=preg_replace("/\.php/",'',$file);
			if ($file) $tmpcontent.='<option value="'.$file.'">'.$file.'</option>'."\n";
		}
	}
	closedir($handle);
}
...
elseif ($this->post['action'] == 'product-import-preview' or (is_numeric($_REQUEST['job_id']) and $_REQUEST['action'] == 'edit_job')) {
	...
	if (is_numeric($_REQUEST['job_id'])) {
		...
		// enable file logging		
		if ($this->get['relaxed_import']) $this->post['relaxed_import']=$this->get['relaxed_import'];		
	}	
	...
	elseif ($this->post['file_url']) {
		...
		if (preg_match("/\.gz$/",$this->post['file_url'])) $filename.='.gz';
	...
	if ((file_exists($file_location) or $this->post['database_name']) and isset($this->post['cid'])) {
		...
		if ($this->post['parser_template']) {
			if (strstr($this->post['parser_template'],"..")) die();
...
<fieldset class="advanced_settings_container hide">
	...
	<input name="add_property" class="importer_add_property" type="button" value="add property" />
				...
				foreach ($rows as $row) {
					...
					$tmpitem=$row;
					$cols=count($tmpitem);	
					if ($this->post['backquotes']) 		$tmpitem[$i]=trim($tmpitem[$i],"\"");
					$alt=$tmpitem[$i];
					if (strlen($tmpitem[$i]) >15) 	$tmpitem[$i]=substr($tmpitem[$i],0,15).'..';
...
//			if ($row['categories_id']) $this->post['cid']=$row['categories_id'];
			...
			if ($log_file) file_put_contents($log_file, $this->HTTP_HOST.' - cron job settings loaded. ('.date("Y-m-d G:i:s").")\n", FILE_APPEND);	
		}			
...
//										$tmpitem[$i] = date("Y-m-d");
							...
							case 'products_deeplink':
								...
								if ($this->post['input'][$i]) $item[$this->post['select'][$i]] .= $this->post['input'][$i];
							break;
								...
								if ($this->post['input'][$i])
								{
									if (strstr($this->post['input'][$i],"|"))
									{
										...
										if ($total >0)
										{
											...
											foreach ($option_values as $option_value)
											{
												if ($subdelimiter)
												{
													...
													$item['attribute_option_value'][]=array($key,$option_value,$option_price);
												}
												else
												{
													$internal_count++;
													...
													if ($internal_count==$total) $internal_count=0;
												}
												$count++;
											}
										}
										...
										if (strstr($tmpitem[$i],"|"))
										{
											...
											{
												$item['attribute_option_value'][]=array($key,$exploded_item);
											}
										}
										else $item['attribute_option_value'][]=array($key,$tmpitem[$i]);
									}
								} else {
									$key='';
									$item['attribute_option_value'][]=array($key,$tmpitem[$i]);
								}								
							break;
							default:
...
//													die();
						...
						*******************/
						if ($this->post['consolidate']) {
							if (@in_array($item['products_name'],$inserteditems[$this->ms['target-cid']])) $skip=1;
							else $skip=0;
						}
						else $skip=0;
						if (!$skip) {
							...
							if ($item['products_name']) {
								...
								if ($item['products_date_added'] and strlen($item['products_date_added'])==4) $item['products_date_added']=$item['products_date_added'].'-01-01';									
								if ($item['products_date_available'] and strlen($item['products_date_available'])==4) $item['products_date_available']=$item['products_date_available'].'-01-01';
							...
							if ($item['products_order_unit_id']) {
								...
								if ($GLOBALS['TYPO3_DB']->sql_num_rows($qry)) {
									...
									// lets add it
									if (!$item['products_order_unit_code'] and $item['products_order_unit_name']) $item['products_order_unit_code']=$item['products_order_unit_name'];
									if (!$item['products_order_unit_name'] and $item['products_order_unit_code']) $item['products_order_unit_name']=$item['products_order_unit_code'];
...
//								error_log('old_product: '.print_r($old_product,1));
								...
								***********************/								
								...
								if ($this->ms['target-cid']) {
									...
									if (isset($item['products_sort_order'])) 	$updateArray['sort_order']=$item['products_sort_order'];
									else										$updateArray['sort_order']=time();									
									$query = $GLOBALS['TYPO3_DB']->INSERTquery('tx_multishop_products_to_categories', $updateArray);
									$res = $GLOBALS['TYPO3_DB']->sql_query($query);										
								}
							...
							if (is_array($item['attribute_option_value'])) {
								foreach ($item['attribute_option_value'] as $option_row) {
									$option_price='';
									if (isset($option_row[2])) 			$option_price=$option_row[2];
									// if option name is defined as an individual col (not through aux)
									if (!$option_row[0] and $item['attribute_option_name']) 	$option_name=$item['attribute_option_name'];
									else														$option_name=$option_row[0];
							...
							while (($row=$GLOBALS['TYPO3_DB']->sql_fetch_assoc($qry)) != false) {
								...
								if ($item[$s]) {
									...
									if ($option_value_id) {
										...
										$str2="SELECT pov.products_options_values_id from tx_multishop_products_options_values_to_products_options povp, tx_multishop_products_options_values pov where povp.products_options_id='".$products_options_id."' and povp.products_options_values_id=pov.products_options_values_id and pov.products_options_values_name='".addslashes($option_value_id)."'";
										$qry2=$GLOBALS['TYPO3_DB']->sql_query($str2);
										$rows2=$GLOBALS['TYPO3_DB']->sql_num_rows($qry2);												
										if (!$rows2) {	
											$str2="INSERT into tx_multishop_products_options_values_to_products_options  (products_options_id,products_options_values_id,sort_order) VALUES ('".$products_options_id."','".$option_value_id."',".time().")";
