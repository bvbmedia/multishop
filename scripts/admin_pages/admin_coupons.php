...
if ($this->post)
{
	if (!$this->post['s_date']) $this->post['s_date']=date("Y-m-d");
	if (!$this->post['e_date']) $this->post['e_date']=date("2020-m-d");
...
if ($this->get['coupons_id']) $title=$this->pi_getLL('edit');
else					$title=$this->pi_getLL('add');
...
if (count($coupons_options) > 0)
{
	...
	foreach ($coupons_options as $option)
	{
		...
		if (!$option['max_usage']) $option['max_usage']=$this->pi_getLL('unlimited');
