...
foreach ($dates as $key => $value)
{
	...
	$where=array();
	if ($this->cookie['negative_keywords_only']) 	$where[]='(s.negative_results=1)';
	else											$where[]='(s.negative_results=0 or s.negative_results=1)';
	...
	while($row=$GLOBALS['TYPO3_DB']->sql_fetch_assoc($qry)) {
		...
		<td width="10">';
		if ($row['negative_results'])	$content.='<span class="negative_icon">x</span>';
		else 							$content.='<span class="positive_icon">+</span>';			
		$content.='</td>
