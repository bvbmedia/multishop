...
if (!$this->ms['MODULES']['GLOBAL_MODULES']['MULTISHOP_VERSION']) {
	...
	$current_version=class_exists('t3lib_utility_VersionNumber')
	? t3lib_utility_VersionNumber::convertVersionNumberToInteger($this->ms['MODULES']['GLOBAL_MODULES']['MULTISHOP_VERSION'])
	: t3lib_div::int_from_ver($this->ms['MODULES']['GLOBAL_MODULES']['MULTISHOP_VERSION']);
					
	$new_version=class_exists('t3lib_utility_VersionNumber')
	? t3lib_utility_VersionNumber::convertVersionNumberToInteger($info['version'])
	: t3lib_div::int_from_ver($info['version']);
...
if (!$GLOBALS["TYPO3_CONF_VARS"]["tx_multishop_started"]) {
	...
	elseif ($product['categories_id']) 	$categories_id=$product['categories_id'];
	if ($categories_id) 				$GLOBALS["TYPO3_CONF_VARS"]['tx_multishop_data']['user_crumbar']=mslib_fe::Crumbar($categories_id);
}
...
$meta_tags=array();	
	...
	jQuery(document).ready(function($){
		...
		else $meta_tags['1_jquery'].='msAdminShortcutFunc();'."\n";
	$meta_tags['1_jquery'].='
	});
	</script>
	';				
