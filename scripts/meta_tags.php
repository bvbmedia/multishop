	...
	if (!$this->ms['MODULES']['GLOBAL_MODULES']['CACHE_FRONT_END'] or ($this->ms['MODULES']['GLOBAL_MODULES']['CACHE_FRONT_END'] and !$html=$Cache_Lite->get($string))) {
		...
		if ($row['total']>0) {
			if ($row['total']==1) {
				...
				$string=sprintf($this->pi_getLL('this_s_there_are_s_orders_created'), $this->pi_getLL('week'), '<strong>'.$row['total'].'</strong>');
			}
			$messages[]='"<a href=\"'.mslib_fe::typolink($this->shop_pid.',2003', 'tx_multishop_pi1[page_section]=admin_orders').'\">'.$string.'</a>"';
		}
		...
		if ($row['total']>0) {
			if ($row['total']==1) {
				$string=sprintf($this->pi_getLL('this_s_there_is_one_order_created'), $this->pi_getLL('month'), '<strong>'.$row['total'].'</strong>');
			} else {
				$string=sprintf($this->pi_getLL('this_s_there_are_s_orders_created'), $this->pi_getLL('month'), '<strong>'.$row['total'].'</strong>');
			}
			$messages[]='"<a href=\"'.mslib_fe::typolink($this->shop_pid.',2003', 'tx_multishop_pi1[page_section]=admin_orders').'\">'.$string.'</a>"';
		}
