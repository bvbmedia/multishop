...
if (!$this->ms['MODULES']['CACHE_FRONT_END'] or ($this->ms['MODULES']['CACHE_FRONT_END'] and !$content=$Cache_Lite->get($string)))
{
	...
	if ($this->get['tx_multishop_pi1']['zip']) $do_search=1;	
	if ($do_search)
	{
		// store search
		$filter		=array();
...
//		$Ycoor=str_replace(",",".",$GLOBALS['TSFE']->fe_user->user['tx_datingsite_current_lng']);	
		...
		if (!$_GET['distance']) $_GET['distance']=1000;
		...
		{
			if (strstr($this->ms['MODULES']['STORES_LISTING_TYPE'],"..")) die('error in STORES_LISTING_TYPE value');
			else 
			{
				if (!$this->ms['MODULES']['STORES_LISTING_TYPE']) $this->ms['MODULES']['STORES_LISTING_TYPE']='default';
				if (strstr($this->ms['MODULES']['STORES_LISTING_TYPE'],"/"))	require($this->DOCUMENT_ROOT.$this->ms['MODULES']['STORES_LISTING_TYPE'].'.php');	
