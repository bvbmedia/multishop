...
if ($this->conf['manufacturers_listing_tmpl_path'])  	$template = $this->cObj->fileResource($this->conf['manufacturers_listing_tmpl_path']);
elseif ($this->conf['manufacturers_listing_tmpl'])  	$template = $this->cObj->fileResource($this->conf['manufacturers_listing_tmpl']);
else													$template = $this->cObj->fileResource(t3lib_extMgm::siteRelPath($this->extKey).'templates/manufacturers_listing.tmpl');
...
if (count($manufacturers) > 0) {
	...
	foreach ($manufacturers as $row) {
		...
		if ($this->ADMIN_USER) $output['admin_manufacturers_sortable_id'] = 'id="sortable_manufacturer_'.$row['manufacturers_id'].'" ';
