<?php
	if ($current['content'])			$content.=mslib_fe::htmlBox($current['categories_name'],$current['content'],1);	
	else								$content.='<div class="main-heading"><h1>'.$current['categories_name'].'</h1></div>';
	...
	foreach ($categories as $category)
	{
		...
		foreach ($products as $item)
		{
			...
			if ($this->ROOTADMIN_USER or ($this->ADMIN_USER and $this->CATALOGADMIN_USER))
			{
				$content.='<div class="admin_menu"><a href="'.mslib_fe::typolink($this->shop_pid.',2002','tx_multishop_pi1[page_section]=admin_ajax&pid='.$item['products_id']).'&action=edit_product" onclick="return hs.htmlExpand(this, { objectType: \'iframe\', width: 910, height: 500} )" class="admin_menu_edit"></a> <a href="'.mslib_fe::typolink($this->shop_pid.',2002','tx_multishop_pi1[page_section]=admin_ajax&pid='.$item['products_id']).'&action=delete_product" onclick="return hs.htmlExpand(this, { objectType: \'iframe\', width: 910, height: 140} )" class="admin_menu_remove" title="Remove"></a></div>';
			}			
...
//			else											$content.='<div style="float:right;color:red">not on stock</div>';					
	...
	$content.='</ul>';	
	if ($current['content_footer'])	$content.= '<div class="msCategoriesFooterDescription">'.mslib_fe::htmlBox('',$current['content_footer'],2) .'</div>';
?>