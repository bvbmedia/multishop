...
if($p > 0) {
	if(($p-1) > 0) {
		$tmp.='<a class="ajax_link pagination_button" href="'.mslib_fe::typolink('', 'p='.($p-1).'&'.mslib_fe::tep_get_all_get_params(array(
					...
					'page',
					'mini_foto',
					'clearcache'))).'">'.$this->pi_getLL('previous').'</a>';
	} else {
		$tmp.='<a class="ajax_link pagination_button" href="'.mslib_fe::typolink('', 'p='.($p-1).'&'.mslib_fe::tep_get_all_get_params(array(
					'p',
					'Submit',
...
for($x=$start_page_number; $x <= $end_page_number; $x++) {
	if(($p+1) == $x) {
		...
		$tmp.='<li><div class="dyna_button"><a class="ajax_link pagination_button" href="'.mslib_fe::typolink('', 'p='.($x-1).'&'.mslib_fe::tep_get_all_get_params(array(
					'p',
					'Submit',
					'page',
					'mini_foto',
					'clearcache'))).'">'.$x.'</a></div></li>';
	}
}
$tmp.='</ul>
</li>';
...
if((($p+1)*$limit_per_page) < $pageset['total_rows']) {
	$tmp.='<a class="ajax_link pagination_button" href="'.mslib_fe::typolink('', 'p='.($p+1).'&'.mslib_fe::tep_get_all_get_params(array(
				...
				'page',
				'mini_foto',
				'clearcache'))).'">'.$this->pi_getLL('next').'</a>';
} else {
...
if((($p+1)*$limit_per_page) < $pageset['total_rows']) {
	...
	$tmp.='<a class="ajax_link pagination_button" href="'.mslib_fe::typolink('', 'p='.$lastpage.'&'.mslib_fe::tep_get_all_get_params(array(
				...
				'page',
				'mini_foto',
				'clearcache'))).'">'.$this->pi_getLL('last').'</a>';
} else {
