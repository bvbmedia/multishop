...
if($p > 0) {
	$tmp .= '<a class="ajax_link pagination_button" href="'.mslib_fe::typolink('',''.mslib_fe::tep_get_all_get_params(array('p','Submit','page','mini_foto','clearcache'))).'">'.$this->pi_getLL('first').'</a>';
} else {
...
if($p > 0) {
	if (($p-1) > 0) {
		$tmp .= '<a class="ajax_link pagination_button" href="'.mslib_fe::typolink('','p='.($p-1).'&'.mslib_fe::tep_get_all_get_params(array('p','Submit','page','mini_foto','clearcache'))).'">'.$this->pi_getLL('previous').'</a>';
	} else {
		$tmp .= '<a class="ajax_link pagination_button" href="'.mslib_fe::typolink('','p='.($p-1).'&'.mslib_fe::tep_get_all_get_params(array('p','Submit','page','mini_foto','clearcache'))).'">'.$this->pi_getLL('previous').'</a>';
	}
} else {
...
if((($p+1)*$limit_per_page) < $pageset['total_rows']) {
	$tmp .= '<a class="ajax_link pagination_button" href="'.mslib_fe::typolink('','p='.($p+1).'&'.mslib_fe::tep_get_all_get_params(array('p','Submit','page','mini_foto','clearcache'))).'">'.$this->pi_getLL('next').'</a>'; 	
} else {
...
if((($p+1)*$limit_per_page) < $pageset['total_rows']) {
	...
	$tmp .= '<a class="ajax_link pagination_button" href="'.mslib_fe::typolink('','p='.$lastpage.'&'.mslib_fe::tep_get_all_get_params(array('p','Submit','page','mini_foto','clearcache'))).'">'.$this->pi_getLL('last').'</a>';
} else{
