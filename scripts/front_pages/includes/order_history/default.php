...
if (mslib_fe::loggedin()) {
	switch ($this->get['tx_multishop_pi1']['page_section']) {
		...
		default:
				...
				if ($order['customer_id']==$GLOBALS['TSFE']->fe_user->user['uid']) {
					foreach ($order['products'] as $product)
					{
						...
						if (is_array($product['attributes']) and count($product['attributes']))
						{
							...
							{
								if ($attribute['products_options_values_id']) 	$value=$attribute['products_options_values_id'];
								else											$value=$attribute['products_options_values'];					
			...
			if ($p >0) $offset=(((($p)*$this->ms['MODULES']['ORDERS_LISTING_LIMIT'])));
			else 
			{
				$p=0;
				$offset=0;
			...
			//		$tmp.='<td align="left" nowrap>'.$order['payment_method_label'].'</td>';
				...
				if (!$this->hidePagination and $pageset['total_rows'] > $this->ms['MODULES']['ORDERS_LISTING_LIMIT'])
				{
					...
					if($p > 0)
					{
						$tmp.=mslib_fe::flexibutton('<a class="pagination_button" href="'.mslib_fe::typolink('',''.mslib_fe::tep_get_all_get_params(array('id','p','Submit','tx_multishop_pi1[action]','clearcache'))).'">'.$this->pi_getLL('first').'</a>');
					}
					else $tmp.='&nbsp;';
					...
					if($p > 0)
					{
						if (($p-1) > 0)
						{
							$tmp.=mslib_fe::flexibutton('<a class="pagination_button" href="'.mslib_fe::typolink('','p='.($p-1).'&'.mslib_fe::tep_get_all_get_params(array('id','p','Submit','tx_multishop_pi1[action]','clearcache'))).'">'.$this->pi_getLL('previous').'</a>','pagenav_previous');
						}
						else
						{
							$tmp.=mslib_fe::flexibutton('<a class="pagination_button" href="'.mslib_fe::typolink('','p='.($p-1).'&'.mslib_fe::tep_get_all_get_params(array('id','p','Submit','tx_multishop_pi1[action]','clearcache'))).'">'.$this->pi_getLL('previous').'</a>','pagenav_previous');
					...
					{
						$tmp.=mslib_fe::flexibutton('<a class="pagination_button" href="'.mslib_fe::typolink('','p='.($p+1).'&'.mslib_fe::tep_get_all_get_params(array('id','p','Submit','tx_multishop_pi1[action]','clearcache'))).'">'.$this->pi_getLL('next').'</a>','pagenav_next');
					} 
					else $tmp.='&nbsp;';	
					...
					{	
						...
						$tmp.=mslib_fe::flexibutton('<a class="pagination_button" href="'.mslib_fe::typolink('','p='.$lastpage.'&'.mslib_fe::tep_get_all_get_params(array('id','p','Submit','tx_multishop_pi1[action]','clearcache'))).'">'.$this->pi_getLL('last').'</a>','pagenav_last');
					}
					else $tmp.='&nbsp;';		
			...
			else $tmp.=$this->pi_getLL('no_orders_found').'.';
			$content.=$tmp;		
		break;
	}
}
