...
if (is_array($products) && count($products)) {
    foreach ($products as $product_index => $current_product) {
		...
		<a href="' . mslib_fe::typolink($this->shop_pid . ',2003', 'tx_multishop_pi1[page_section]=delete_product&cid=' . $current_product['categories_id'] . '&pid=' . $current_product['products_id'] . '&action=delete_product&cid=' . $current_product['categories_id'], 1) . '" class="admin_menu_remove" title="Remove"><i class="fa fa-trash-o"></i></a>
		</div>';
        }
        $markerArray = array();
