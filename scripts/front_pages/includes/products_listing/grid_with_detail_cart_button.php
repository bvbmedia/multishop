...
if (!defined('TYPO3_MODE')) die ('Access denied.');
	...
	foreach ($products as $current_product)
	{
		...
		if ($current_product['products_url']) 	$link=$current_product['products_url'];
		else									$link=mslib_fe::typolink($this->conf['products_detail_page_pid'],'&'.$where.'&products_id='.$current_product['products_id'].'&tx_multishop_pi1[page_section]=products_detail');
