...
if (!$product['products_id']) {
	...
	if(!$this->conf['disableMetatags'])	{
		...
		} else if ($product['products_shortdescription']) {
	...
	if ($product['maximum_quantity'] > 0 || (is_numeric($product['products_multiplication']) && $product['products_multiplication'] > 0)) {
		...
		} else if($product['products_multiplication']) {
		...
		} else if ($product['maximum_quantity'] && !$product['products_multiplication']) {
	...
	} else if ($this->conf['product_detail_tmpl']) {
