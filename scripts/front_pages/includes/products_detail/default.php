...
if (!$product['products_id']) {
	...
	if ($this->ms['MODULES']['AFFILIATE_SHOP'] and $product['products_url']) {
		if (!strstr($product['products_url'],'http://') and !strstr($product['products_url'],'http://')) $product['products_url']='http://'.$product['products_url'];
		...
		if (($this->ROOTADMIN_USER or ($this->ADMIN_USER and $this->CATALOGADMIN_USER)) and !$product['products_status'] and !$this->ms['MODULES']['FLAT_DATABASE']) {
	...
	elseif ($this->conf['product_detail_tmpl'])  	$template = $this->cObj->fileResource($this->conf['product_detail_tmpl']);	
	else											$template = $this->cObj->fileResource(t3lib_extMgm::siteRelPath($this->extKey).'templates/products_detail.tmpl');
