		...
		<ul id="multishop_payment_method">';
			...
			foreach ($payment_methods as $code => $item) {
				...
				if ($price > 0) {
					...
					if (!strstr($price,"%")) $price=mslib_fe::amount2Cents($price);				
		...
		<div class="content">
		 <ul id="multishop_shipping_method">';
			...
			foreach ($shipping_methods as $code => $item) {
				...
				if ($price_wrap) $content.=$price_wrap;
				...
				if ($item['description']) $content.='<span class="description">'.$item['description'].'</span>';
				$content.='</label>';				
				$content.='</li>';			
			}
		$content.='
	...
	{	
		...
		{			
			if ($xxi==0)
			{
				...
				if ($hide) 	$jquery_content.='jQuery(\'#shipping_method\').hide(\'slow\', function(){});';
/*				
				else		$jquery_content.='
...
*/				
			...
			if ($hide) 	$jquery_content.='jQuery(\'#shipping_method\').hide(\'slow\', function(){});';
			else		$jquery_content.='jQuery(\'#shipping_method\').show(\'slow\', function(){});';
			$jquery_content.='
				});
			';
			$xxi++;
