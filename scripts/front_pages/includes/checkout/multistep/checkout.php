...
if (is_array($cart['products']) and count($cart['products']))
{
	...
	if ($this->ms['MODULES']['PRODUCT_EDIT_METHOD_FILTER'])
	{
		...
		{
			if (is_numeric($array['products_id'])) $pids[]=$array['products_id'];
		}
		if(count($pids))
		{
...
else $stepCodes = array('checkout_address', 'checkout_shipping_payment_method', 'checkout_review', 'checkout_finished'); 
...
{	
	...
	for ($i=0;$i<count($steps);$i++)
	{
		...
		if ($steps[$i] == $current) $output.='<strong>';
...
if ($this->get['tx_multishop_pi1']['previous_checkout_section'])
{
	for ($i=1;$i<=count($stepCodes);$i++)
	{
		...
		{
			if ($this->post) $posted_page=current($stepCodes);
			break;				
		}
		elseif (!next($stepCodes)) break;
	}
}	
$content.='';
