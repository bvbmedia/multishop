...
if (!defined('TYPO3_MODE')) die ('Access denied.');
	...
	foreach ($rel_products as $rel_rs)
	{
		...
		if ($this->ms['MODULES']['SHOW_STOCK_LEVEL_AS_BOOLEAN'] != 'no')
		{
			...
			{
				case 'yes_with_image':
					if ($rel_rs['products_quantity']) 	$rel_rs['products_quantity']='<img src="'.t3lib_extMgm::siteRelPath($this->extKey).'templates/images/icons/status_green.png" alt="'.htmlspecialchars($this->pi_getLL('in_stock')).'" />';
					else								$rel_rs['products_quantity']='<img src="'.t3lib_extMgm::siteRelPath($this->extKey).'templates/images/icons/status_red.png" alt="'.htmlspecialchars($this->pi_getLL('not_in_stock')).'" />';
				break;
				case 'yes_without_image':
					if ($rel_rs['products_quantity']) 	$rel_rs['products_quantity']=$this->pi_getLL('admin_yes');
					else								$rel_rs['products_quantity']=$this->pi_getLL('admin_no');
				break;
			}
		}
		if ($rel_rs['products_image']) $image='<img src="'.mslib_befe::getImagePath($rel_rs['products_image'],'products','50').'" alt="'.htmlspecialchars($rel_rs['products_name']).'" />';
		else $image='<div class="no_image_50"></div>';			
