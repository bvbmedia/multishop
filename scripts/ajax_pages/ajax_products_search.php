...
//				if (strlen($item) < 4)
	...
	{
		if (strstr($this->ms['MODULES']['PRODUCTS_LISTING_AUTO_COMPLETE_TYPE'],"..")) die('error in PRODUCTS_LISTING_AUTO_COMPLETE_TYPE value');
		else 
		{
			if (strstr($this->ms['MODULES']['PRODUCTS_LISTING_AUTO_COMPLETE_TYPE'],"/"))	require($this->DOCUMENT_ROOT.$this->ms['MODULES']['PRODUCTS_LISTING_AUTO_COMPLETE_TYPE'].'.php');	
			else	require(t3lib_extMgm::extPath('multishop').'scripts/ajax_pages/includes/products_listing_auto_complete/'.$this->ms['MODULES']['PRODUCTS_LISTING_AUTO_COMPLETE_TYPE'].'.php');	
		}			
	}
	else
	{
