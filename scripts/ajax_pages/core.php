    ...
    case 'admin_dashboards':
        ...
        switch ($this->get['tx_multishop_pi1']['action']) {
            case 'save_dashboard_layout':
                $updateArray = array();
                $updateArray['dashboard_layout'] = $this->post['tx_multishop_pi1']['new_layout'];
                ...
                break;
            case 'add_new_widget':
                if (isset($this->post['tx_multishop_pi1']['widget_key']) && !empty($this->post['tx_multishop_pi1']['widget_key'])) {
                    $widget_key=$this->post['tx_multishop_pi1']['widget_key'];
                    ...
                    switch ($widget_key) {
                        ...
                        default:
                            ...
                            if (is_array($GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['ext/multishop/scripts/ajax_pages/core.php']['compileWidgetDefault'])) {
                                $params = array('key' => &$key, 'compiledWidget' => &$compiledWidget);
                                foreach ($GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['ext/multishop/scripts/ajax_pages/core.php']['compileWidgetDefault'] as $funcRef) {
                                    \TYPO3\CMS\Core\Utility\GeneralUtility::callUserFunction($funcRef, $params, $this);
                                }
                            }
                            break;
                ...
                if (is_array($compiledWidget) && count($compiledWidget) && is_numeric($this->post['tx_multishop_pi1']['dashboard_id'])) {
                    $insertArray = array();
                    $insertArray['header_title'] = $compiledWidget['title'];
                    $insertArray['dashboard_id'] = $this->post['tx_multishop_pi1']['dashboard_id'];
                    $insertArray['widget_key'] = $widget_key;
                    $insertArray['colpos'] = $compiledWidget['defaultCol'];
                    $insertArray['sort_order'] = '1';
                    $insertArray['crdate'] = time();
                    $insertArray['status'] = 1;
                    $query = $GLOBALS['TYPO3_DB']->INSERTquery('tx_multishop_dashboard_portlets', $insertArray);
                    $return_data['status']='NOTOK';
                    $return_data['reason']='Duplicate widget';
                    if ($res = $GLOBALS['TYPO3_DB']->sql_query($query)) {
                        $return_data['status']='OK';
                        $return_data['reason']='';
                    }
                ...
                break;
            case 'save_widget_sort':
                if (is_array($this->post['tx_multishop_pi1']['widgets_sort']) && count($this->post['tx_multishop_pi1']['widgets_sort'])) {
                    $row_counter=1;
                    foreach ($this->post['tx_multishop_pi1']['widgets_sort'] as $widgetsSort) {
                        $col_counter=1;
                        foreach ($widgetsSort as $widget_key) {
                            if ($widget_key) {
                                $updateArray = array();
                                $updateArray['colpos'] = $row_counter;
                                $updateArray['sort_order'] = $col_counter;
                                $query = $GLOBALS['TYPO3_DB']->UPDATEquery('tx_multishop_dashboard_portlets', 'widget_key=\'' . addslashes($widget_key) . '\'', $updateArray);
                                $res = $GLOBALS['TYPO3_DB']->sql_query($query);
                                $col_counter++;
                            }
                        }
