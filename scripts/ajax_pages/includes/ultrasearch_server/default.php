...
//			$where[]	=$tbl."products_name LIKE '%$key%' ";	
	...
	{
		if (!$this->ms['MODULES']['FLAT_DATABASE']) 	$having[]="(final_price BETWEEN '".$this->post['min']."' and '".$this->post['max']."')";
		else 									$filter[]="(pf.final_price BETWEEN '".$this->post['min']."' and '".$this->post['max']."')";
	}
	...
	if ($test_orderby)
	{
		switch($test_orderby)
		{
			...
			case 'products_name DESC':
				if ($test_orderby=='products_name DESC') $sort='desc';
				else									 $sort='asc';
				if (!$this->ms['MODULES']['FLAT_DATABASE']) 
				{			
					$prefix='p.';
			...
			case 'final_price DESC':
				if ($test_orderby=='final_price DESC') 	$sort='desc';
				else									$sort='asc';		
...
//	error_log($this->ms['MODULES']['PRODUCTS_LISTING_LIMIT']);
	if ($pageset['total_rows'] > 0)
	{
		...
		if (count($products))
		{
			...
			foreach($products as $index => $product)
			{
				...
				if ($product['products_price'] <> $product['final_price'])
				{
					...
					else $old_price=$product['products_price'];	
				...
				if ($product['products_image']) 	$temp_var_products['products_image']=mslib_befe::getImagePath($product['products_image'],'products','100');
				if ($product['products_image1']) 	$temp_var_products['products_image1']=mslib_befe::getImagePath($product['products_image1'],'products','100');
		...
		{
			// no results
			if ($this->post['skeyword']) mslib_befe::storeProductsKeywordSearch($this->post['skeyword'],'1');
		}
	}
