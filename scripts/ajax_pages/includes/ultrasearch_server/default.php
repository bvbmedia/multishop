        ...
        if (count($products)) {
            ...
            foreach ($products as $index => $product) {
                ...
                if ($this->ROOTADMIN_USER or ($this->ADMIN_USER and $this->CATALOGADMIN_USER)) {
                    ...
                    $temp_var_products['admin_delete_product'] = mslib_fe::typolink($this->shop_pid . ',2003', 'tx_multishop_pi1[page_section]=delete_product&cid=' . $product['categories_id'] . '&pid=' . $product['products_id'] . '&action=delete_product&cid=' . $product['categories_id'], 1);
                }
                if ($this->ms['MODULES']['FLAT_DATABASE']) {
                    for ($s = 0; $s < 5; $s++) {
