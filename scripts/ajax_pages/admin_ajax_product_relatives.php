...
if ($this->post['req'] == 'init') {
	...
	$query = $GLOBALS['TYPO3_DB']->SELECTquery(
			'products_id, relative_product_id',         // SELECT ...
	...
	while ($rows = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
		...
		} else if ($rows['products_id'] != $product['products_id']) {
	...
	//die($where);
	$query = $GLOBALS['TYPO3_DB']->SELECTquery(
		'B.categories_id,C.categories_name',         // SELECT ...
		...
		while(($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) != false) {
			if ($row['categories_name']) {
				...
				$query2 = $GLOBALS['TYPO3_DB']->SELECTquery(
					'A.products_id, A.products_name, B.categories_id,C.categories_name',         // SELECT ...
					'tx_multishop_products p, tx_multishop_products_description A INNER JOIN tx_multishop_products_to_categories B ON A.products_id = B.products_id INNER JOIN tx_multishop_categories_description C ON B.categories_id = C.categories_id',     // FROM ...
	...
	if (strpos($this->post['product_id'], '&') !== false) {
		...
		foreach ($data_pids as $data_pid) {
			...
			if ($row_check['total'] == 0){
				//echo $row_check['jum'];
				$updateArray = array();
				$updateArray =array("products_id"=>$this->post['pid'], "relative_product_id"=>$data_pid);
		...
		if ($row_check['total'] == 0){
			//echo $row_check['jum'];
			$updateArray = array();
			$updateArray =array("products_id"=>$this->post['pid'], "relative_product_id"=>$this->post['product_id']);
			$query = $GLOBALS['TYPO3_DB']->INSERTquery('tx_multishop_products_to_relative_products',$updateArray);
			$res = $GLOBALS['TYPO3_DB']->sql_query($query);
			if ($res){
				$json_data['saved'] = "OK";
