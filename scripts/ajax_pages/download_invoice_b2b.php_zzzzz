...
//					$price=round(($options['options_values_price']*($product['products_tax']/100))+$options['options_values_price'],4);
		...
		if ($this->ms['MODULES']['SHOW_PRICES_INCLUDING_VAT']) {
			//$final_price=round(($product['final_price']*($product['products_tax']/100))+$product['final_price'],4);
		...
		$pdfdata['data'][]=array(
			$prefix.number_format($product['qty']),
...
//				$this->Rect($x, $y, $w, $h);
	...
	$pdf->SetFont('Arial', '', 9);
	$pdf->Row(array(
		utf8_decode(ucfirst($this->pi_getLL('qty'))),
		utf8_decode(ucfirst($this->pi_getLL('products_model'))),
