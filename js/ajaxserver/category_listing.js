...
function filterproducts ()
		...
		jQuery(".option-attributes").each(
		 ...
		 jQuery("#ajax_ultrasearch").submit( function() {
			 alert("OK");
		 });
			 	jQuery.ajax(
					{
						type: "POST",
					   	url: "index.php?eID=multishop&type=ajaxserver_category_listing",
					   	cache :false,
					   	data: query_string,
					   	success: 
						    function(t) 
...
jQuery(document).ready(function()
		{
				...
				pages.click(function(){
					...
					//Load content
					var pageNum = this.id;
					var targetUrl = "index.php?eID=multishop&type=ajaxserver_category_listing";
					query_string += "&min=" + jQuery("#Filter_5_Min").val();
					query_string += "&max=" + jQuery("#Filter_5_Max").val();
