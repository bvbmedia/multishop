...
switch ($_REQUEST['action'])
{
	...
	if ($_FILES['restore_configuration_file']['tmp_name'])
	{
		...
		if (count($rows) >0 and $target_pid)
		{
			foreach ($rows as $row)
			{
				...
				{
					if ($key == 'page_uid') $array[$key] = $target_pid;						
					elseif ($key != 'id') 	$array[$key] = $val;
				}
				$records[]=$array;				
			}
			if (count($records) >0)
	...
	case 'image_resizer':
		...
		while(($row = $GLOBALS['TYPO3_DB']->sql_fetch_row($res)) != false) if ($row[0]) $data['categories'][] = $row[0];
	...
	foreach ($paths as $path) if (!is_dir($path)) t3lib_div::mkdir($path,0766);	
	break;
	case 'erase':
	...
	if (is_numeric($_GET['page_uid']))
	{
		...
		while(($row = $GLOBALS['TYPO3_DB']->sql_fetch_row($res)) != false) $data['categories_id'][]	= $row[0];
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('products_id', 'tx_multishop_products', 'page_uid='.$_GET['page_uid'], '');
		while(($row = $GLOBALS['TYPO3_DB']->sql_fetch_row($res)) != false) $data['products_id'][]	= $row[0];
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('manufacturers_id', 'tx_multishop_manufacturers', 'page_uid='.$_GET['page_uid'], '');
		while(($row = $GLOBALS['TYPO3_DB']->sql_fetch_row($res)) != false) $data['manufacturers_id'][]	= $row[0];
...
//					{
					...
					elseif ($paths[1] == 'images' 	&& 	$paths[2] == 'products_extra_images' 	&& $paths[3])	$restore_files['products'][$paths[3]]=$fullpath.'/'.$path;
					elseif ($paths[1] == 'images' 	&& 	$paths[2] == 'categories' 				&& $paths[3]) 	$restore_files['categories'][$paths[3]]=$fullpath.'/'.$path;
					elseif ($paths[1] == 'images' 	&& 	$paths[2] == 'manufacturers'			&& $paths[3]) 	$restore_files['manufacturers'][$paths[3]]=$fullpath.'/'.$path;
//					}					
				}
			}		
				...
				if (is_array($database['customers']) and count($database['customers'])) {
					...
					foreach ($database['customers'] as $row) {
						...
						if ($user['uid']) {
							...
							if ($row['customers_dob'] and $row['customers_dob'] != '0000-00-00 00:00:00')	 $insertArray['date_of_birth']	=strtotime($row['customers_dob']);
...
//			die();
			foreach ($database as $key => $records)
			{
				...
				if ($this->post['full_restore'])
				{
					...
					$qry = $GLOBALS['TYPO3_DB']->sql_query($str);		
					if ($qry) $content.= '<BR>Table cleared and is now empty.';
					// CLEAR TABLES EOF	
					foreach ($records as $record)
					{
						if (array_key_exists('page_uid',$record)) 		$record['page_uid']=$this->post['page_uid'];
						switch ($key)
						{
							case 'tx_multishop_products':
								if ($record['products_date_added']>0)	 $record['products_date_added']=strtotime($record['products_date_added']);
								if ($record['products_last_modified']>0) $record['products_last_modified']=strtotime($record['products_last_modified']);
								if ($record['products_date_available']>0) $record['products_date_available']=strtotime($record['products_date_available']);
							break;
						}
...
$record['products_status']=1;
		...
		else $content.='File is not valid. Restore didn\'t succeed.';
		$content.='</fieldset>';
		// now remove the backup folder
		if ($fullpath)	$tmp=$this->deltree($fullpath);		
	}
	...
	if ($_GET['action']=='backup' and is_numeric($_GET['page_uid']))
	{
		...
		if (!file_put_contents(PATH_site.'uploads/tx_multishop/tmp/'.$backup_folder.'/data.sql',$output)) die(PATH_site.'uploads/tx_multishop/tmp/'.$backup_folder.'/data.sql is not writable.');
		else
		{
			...
			if ($this->zipPack(PATH_site.'uploads/tx_multishop/tmp/'.$backup_folder.'/', PATH_site.'uploads/tx_multishop/tmp/'.$backup_folder.'/'.$backup_file.'.zip'))
			{
				...
				// now remove the backup folder
				if ($backup_folder)	$tmp=$this->deltree(PATH_site.'uploads/tx_multishop/tmp/'.$backup_folder);
				ob_end_clean();
...
$multishop_content_objects = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
	'*',
	'tt_content',
	'deleted=0 and hidden=0 and sys_language_uid=0 and list_type = \'multishop_pi1\' and pi_flexform like \'%<value index="vDEF">coreshop</value>%\' and pi_flexform like \'%<field index="page_uid">
                    <value index="vDEF"></value>
                </field>%\'',
	''
);
...
if (count($multishop_content_objects) > 0)
{
	...
	{				
		...
		if (is_numeric($pageinfo['uid']))
		{	
			if (!$shops[$pageinfo['uid']])	$shops[$pageinfo['uid']]=0;
		}
	}
}
if (count($shops) >0)
{
	...
	foreach ($shops as $page_uid => $total)
	{
		...
		if ($pageinfo['uid']) 	$label=$pageinfo['_thePathFull'];
		else					$label='Unknown (pid: '.$page_uid.')';
		foreach ($shops as $page_uid2 => $total2)
		{	
			$pageinfo2 = t3lib_BEfunc::readPageAccess($page_uid2,'');
			if ($pageinfo2['uid']) $label2=$pageinfo2['_thePathFull'];
			else							$label2='Unknown (pid: '.$page_uid2.')';
			if ($page_uid2 != $page_uid and $total >0) 	$options.='<option value="'.$page_uid.'_'.$page_uid2.'">Copy ['.$label.': '.$total.' records] to ['.$label2.': '.$total2.' records]</option>'."\n";
		}
	}
	if ($options)
	{
