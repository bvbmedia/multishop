				...
				if ($send_mail) {
					if ($user['email']) {
						if (strpos($mail_template, 'email_order_paid_letter')!==false && $this->ms['MODULES']['ATTACH_INVOICE_PDF_IN_PAID_LETTER_EMAIL']>0) {
							...
							if ($order_invoice['orders_id'] &&!file_exists($pdfFilePath)) {
								$orders_tax_data=$order_invoice['orders_tax_data'];
								if ($this->sys_language_uid != $order_invoice['sys_language_uid']) {
									// The order is created in different language, let us try to find the correct localized label
									$methodLabel=mslib_befe::getShippingMethodLabelByCode($order_invoice['shipping_method'],$this->sys_language_uid);
									if ($methodLabel) {
										$order_invoice['shipping_method_label']=$methodLabel;
									}
								}
								if ($this->sys_language_uid != $order_invoice['sys_language_uid']) {
									// The order is created in different language, let us try to find the correct localized label
									$methodLabel=mslib_befe::getPaymentMethodLabelByCode($order_invoice['payment_method'],$this->sys_language_uid);
									if ($methodLabel) {
										$order_invoice['payment_method_label']=$methodLabel;
									}
								}
								//hook to let other plugins further manipulate the replacers
								...
								$markerArray=array();
								$markerArray['###GENDER_SALUTATION###']=mslib_fe::genderSalutation($order_invoice['billing_gender']);
								...
								// billing address
								if (!empty($order_invoice['billing_company'])) {
									$markerArray['###BILLING_COMPANY###']='<strong>'.$order_invoice['billing_company'].'</strong><br/>';
								} else {
									$markerArray['###BILLING_COMPANY###']='';
								}
								$markerArray['###BILLING_NAME###']=$order_invoice['billing_name'];
								$markerArray['###BILLING_ADDRESS###']=$order_invoice['billing_address'];
								$markerArray['###BILLING_ZIP###']=$order_invoice['billing_zip'];
								$markerArray['###BILLING_CITY###']=mslib_befe::strtoupper($order_invoice['billing_city']);
								$markerArray['###BILLING_COUNTRY###']='';
								if (mslib_befe::strtolower($order_invoice['billing_country'])!=mslib_befe::strtolower($this->tta_shop_info['country'])) {
									// ONLY PRINT COUNTRY IF THE COUNTRY OF THE CUSTOMER IS DIFFERENT THAN FROM THE SHOP
									$markerArray['###BILLING_COUNTRY###']=mslib_fe::getTranslatedCountryNameByEnglishName($this->lang, $order_invoice['billing_country']).'<br/>';
								}
								// delivery address
								if (!empty($order_invoice['delivery_company'])) {
									$markerArray['###DELIVERY_COMPANY###']='<strong>'.$order_invoice['delivery_company'].'</strong><br/>';
								} else {
									$markerArray['###DELIVERY_COMPANY###']='';
								}
								$markerArray['###DELIVERY_NAME###']=$order_invoice['delivery_name'];
								$markerArray['###DELIVERY_ADDRESS###']=$order_invoice['delivery_address'];
								$markerArray['###DELIVERY_ZIP###']=$order_invoice['delivery_zip'];
								$markerArray['###DELIVERY_CITY###']=mslib_befe::strtoupper($order_invoice['delivery_city']);
								$markerArray['###DELIVERY_COUNTRY###']='';
								if (mslib_befe::strtolower($order_invoice['delivery_country'])!=mslib_befe::strtolower($this->tta_shop_info['country'])) {
									// ONLY PRINT COUNTRY IF THE COUNTRY OF THE CUSTOMER IS DIFFERENT THAN FROM THE SHOP
									$markerArray['###DELIVERY_COUNTRY###']=mslib_fe::getTranslatedCountryNameByEnglishName($this->lang, $order_invoice['delivery_country']).'<br/>';
								}
								$markerArray['###LABEL_CUSTOMER_ID###']=$this->pi_getLL('admin_customer_id');
								$markerArray['###CUSTOMER_ID###']=$order_invoice['customer_id'];
								...
								$markerArray['###ORDER_DATE###']=strftime("%x", $order_invoice['crdate']);
								...
								if ($order_invoice['shipping_method_label']) {
									$markerArray['###LABEL_INVOICE_SHIPPING_METHOD###']=$this->pi_getLL('shipping_method');
									$markerArray['###INVOICE_SHIPPING_METHOD###']=$order_invoice['shipping_method_label'];
								}
								$markerArray['###LABEL_INVOICE_PAYMENT_METHOD###']='';
								$markerArray['###INVOICE_PAYMENT_METHOD###']='';
								if ($order_invoice['payment_method_label']) {
									$markerArray['###LABEL_INVOICE_PAYMENT_METHOD###']=$this->pi_getLL('payment_method');
									$markerArray['###INVOICE_PAYMENT_METHOD###']=$order_invoice['payment_method_label'];
								}
								$markerArray['###INVOICE_ORDER_DETAILS###']=mslib_befe::printInvoiceOrderDetailsTable($order_invoice, $invoice['invoice_id'], $prefix);
								$markerArray['###LABEL_YOUR_VAT_ID###']='';
								$markerArray['###YOUR_VAT_ID###']='';
								if ($order_invoice['billing_vat_id']) {
									$markerArray['###LABEL_YOUR_VAT_ID###']=$this->pi_getLL('your_vat_id');
									$markerArray['###YOUR_VAT_ID###']=strtoupper($order_invoice['billing_vat_id']);
								}
								$markerArray['###LABEL_YOUR_COC_ID###']='';
								$markerArray['###YOUR_COC_ID###']='';
								if ($order_invoice['billing_coc_id']) {
									$markerArray['###LABEL_YOUR_COC_ID###']=$this->pi_getLL('your_coc_id');
									$markerArray['###YOUR_COC_ID###']=strtoupper($order_invoice['billing_coc_id']);
								}
								$markerArray['###BILLING_TELEPHONE###']='';
								if (!empty($order_invoice['billing_telephone'])) {
									$markerArray['###BILLING_TELEPHONE###']=$order_invoice['billing_telephone'].'<br/>';
								}
								$markerArray['###BILLING_MOBILE###']='';
								if (!empty($order_invoice['billing_mobile'])) {
									$markerArray['###BILLING_MOBILE###']=$order_invoice['billing_mobile'].'<br/>';
								}
								$markerArray['###DELIVERY_TELEPHONE###']='';
								if (!empty($order_invoice['delivery_telephone'])) {
									$markerArray['###DELIVERY_TELEPHONE###']=$order_invoice['delivery_telephone'].'<br/>';
								}
								$markerArray['###DELIVERY_MOBILE###']='';
								if (!empty($order_invoice['delivery_mobile'])) {
									$markerArray['###DELIVERY_MOBILE###']=$order_invoice['delivery_mobile'].'<br/>';
								}
								$markerArray['###CUSTOMER_COMMENTS###']=$order_invoice['customer_comments'];
								// CMS HEADER
								$markerArray['###INVOICE_CONTENT_HEADER_MESSAGE###']='';
								$cmsKeys=array();
								if ($order_invoice['payment_method']) {
									$cmsKeys[]='pdf_invoice_header_message_'.$order_invoice['payment_method'];
								}
								...
								foreach ($cmsKeys as $cmsKey) {
									$page_invoice=mslib_fe::getCMScontent($cmsKey, $GLOBALS['TSFE']->sys_language_uid);
									if (!empty($page_invoice[0]['content'])) {
										$markerArray['###INVOICE_CONTENT_HEADER_MESSAGE###']='<div class="content_header_message">
										<br/><br/><br/>
										'.$page_invoice[0]['content'].'
										</div>';
									}
									if (is_array($page_invoice)) {
										break;
									}
								}
								// CMS FOOTER
								...
								$cmsKeys=array();
								if ($order_invoice['payment_method']) {
									$cmsKeys[]='pdf_invoice_footer_message_'.$order_invoice['payment_method'];
								}
								...
								$array2=array();
								$array1[]='###BILLING_FULL_NAME###';
								$array2[]=$order_invoice['billing_name'];
								$array1[]='###FULL_NAME###';
								$array2[]=$order_invoice['billing_name'];
								$array1[]='###BILLING_NAME###';
								$array2[]=$order_invoice['billing_name'];
								$array1[]='###BILLING_FIRST_NAME###';
								$array2[]=$order_invoice['billing_first_name'];
								$array1[]='###BILLING_LAST_NAME###';
								$array2[]=preg_replace('/\s+/', ' ', $order_invoice['billing_middle_name'].' '.$order_invoice['billing_last_name']);
								$array1[]='###BILLING_EMAIL###';
								$array2[]=$order_invoice['billing_email'];
								$array1[]='###BILLING_TELEPHONE###';
								$array2[]=$order_invoice['billing_telephone'];
								$array1[]='###BILLING_MOBILE###';
								$array2[]=$order_invoice['billing_mobile'];
								// full delivery name
								$array1[]='###DELIVERY_NAME###';
								$array2[]=$order_invoice['delivery_name'];
								$array1[]='###DELIVERY_FULL_NAME###';
								$array2[]=$order_invoice['delivery_name'];
								$array1[]='###DELIVERY_FIRST_NAME###';
								$array2[]=$order_invoice['delivery_first_name'];
								$array1[]='###DELIVERY_LAST_NAME###';
								$array2[]=preg_replace('/\s+/', ' ', $order_invoice['delivery_middle_name'].' '.$order_invoice['delivery_last_name']);
								$array1[]='###DELIVERY_EMAIL###';
								$array2[]=$order_invoice['delivery_email'];
								$array1[]='###DELIVERY_TELEPHONE###';
								$array2[]=$order_invoice['delivery_telephone'];
								$array1[]='###DELIVERY_MOBILE###';
								$array2[]=$order_invoice['delivery_mobile'];
								$array1[]='###CUSTOMER_EMAIL###';
								$array2[]=$order_invoice['billing_email'];
								$time=$order_invoice['crdate'];
								$long_date=strftime($this->pi_getLL('full_date_format'), $time);
								$array1[]='###ORDER_DATE_LONG###'; // ie woensdag 23 juni, 2010
								$array2[]=$long_date;
								// backwards compatibility
								...
								$array2[]=$this->ms['MODULES']['STORE_NAME'];
								$array1[]='###TOTAL_AMOUNT###';
								$array2[]=mslib_fe::amount2Cents($prefix.$order_invoice['total_amount']);
								$array1[]='###PROPOSAL_NUMBER###';
								$array2[]=$order_invoice['orders_id'];
								$array1[]='###ORDER_NUMBER###';
								$array2[]=$order_invoice['orders_id'];
								$array1[]='###ORDER_LINK###';
								$array2[]='';
								$array1[]='###ORDER_STATUS###';
								$array2[]=$order_invoice['orders_status'];
								$array1[]='###TRACK_AND_TRACE_CODE###';
								$array2[]=$order_invoice['track_and_trace_code'];
								$array1[]='###BILLING_ADDRESS###';
								$array2[]=$billing_address;
								$array1[]='###DELIVERY_ADDRESS###';
								$array2[]=$delivery_address;
								$array1[]='###CUSTOMER_ID###';
								$array2[]=$order_invoice['customer_id'];
								$array1[]='###INVOICE_NUMBER###';
								...
								$array2[]=$ORDER_DETAILS;
								$array1[]='###SHIPPING_METHOD###';
								$array2[]=$order_invoice['shipping_method_label'];
								$array1[]='###PAYMENT_METHOD###';
								$array2[]=$order_invoice['payment_method_label'];
								$array1[]='###EXPECTED_DELIVERY_DATE###';
								$array2[]=strftime("%x", $order_invoice['expected_delivery_date']);
								$array1[]='###CUSTOMER_COMMENTS###';
								$array2[]=$order_invoice['customer_comments'];
								$array1[]='###PAYMENT_CONDITION###';
								if ($order_invoice['payment_condition']) {
									$array2[]=$order_invoice['payment_condition'].' '.$this->pi_getLL('days');
								} else {
									$array2[]='';
								}
								$array1[]='###PAYMENT_DUE_DATE###';
								if ($order_invoice['payment_condition']) {
									$array2[]=strftime("%x", strtotime('+'.$order_invoice['payment_condition'].' day', $invoice['crdate']));
								} else {
									$array2[]='';
								}
								$array1[]='###GENDER_SALUTATION###';
								$array2[]=mslib_fe::genderSalutation($order_invoice['billing_gender']);

								$markerArray['###PAID_STATUS_LABEL###']=$this->pi_getLL('order_payment_status');
								if ($order_invoice['paid']) {
									$array1[]='###PAID_STATUS###';
								...
								if ($order_invoice['paid']) {
									$array1[]='###PAID_DATE###';
									$array2[]=strftime("%x", $order_invoice['orders_paid_timestamp']);
									$markerArray['###PAID_DATE###']=strftime("%x", $order_invoice['orders_paid_timestamp']);
								} else {
									$array1[]='###PAID_DATE###';
									$array2[]=$this->pi_getLL('unpaid');
								...
								if ($order_invoice['orders_paid_timestamp']) {
									$date=strftime("%x", $order_invoice['orders_paid_timestamp']);
									// add the label
								...
								if ($order_invoice['payment_condition']) {
									...
									$markerArray['###INVOICE_PAYMENT_CONDITION###']=$order_invoice['payment_condition'].' '.$this->pi_getLL('days');
								}
								// MARKERS EOL
								$invoice_content=$this->cObj->substituteMarkerArray($template, $markerArray);
								...
								$dompdf=new DOMPDF();
								$dompdf->set_paper('A4');
								$dompdf->load_html($invoice_content, 'UTF-8');
								$dompdf->render();
