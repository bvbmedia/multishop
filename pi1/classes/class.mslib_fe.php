...
class mslib_fe
{
	...
	public function image_type_to_extension($imagetype,$include_dot=false) {
		 ...
		 switch($imagetype) {
			  ...
			  case 1 	: return $dot.'gif'; 
			  case 2 	: return $dot.'jpg'; 
			  case 3 	: return $dot.'png';
			  case 4 	: return $dot.'swf';
			  case 5 	: return $dot.'psd';
			  case 6 	: return $dot.'jpg';
			  case 7 	: return $dot.'tiff';
			  case 8 	: return $dot.'tiff';
			  case 9 	: return $dot.'jpc';
			  case 10 	: return $dot.'jp2';
			  case 11 	: return $dot.'jpx';
			  case 12 	: return $dot.'jb2';
			  case 13 	: return $dot.'swc';
			  case 14 	: return $dot.'aiff';
			  case 15 	: return $dot.'wbmp';
			  case 16 	: return $dot.'xbm';
	...
	public function normaliza ($string){    
	  $normalizeChars = array(
	   ...
	   'ú'=>'u', 'û'=>'u', 'ý'=>'y', 'ý'=>'y', 'þ'=>'b', 'ÿ'=>'y', 'ƒ'=>'f', 'ü' => 'u','š' => 's', 'd' => 'd',
	   'c'=>'c', 'c'=>'c', 'ž'=>'z', 'Š'=>'s', 'Ð'=>'d', 'C'=>'c', 'C'=>'c', 'Ž' => 'z'
	  );
...
//			$mail->MsgHTML($body,$this->DOCUMENT_ROOT);
	...
	public function Crumbar ($c,$languages_id='',$output=array()) {		
		if (is_numeric($c)) {
			if ($this->ms['MODULES']['CACHE_FRONT_END']) {
				...
				if (!count($output) and $this->ms['MODULES']['CACHE_TIME_OUT_CRUM']) {
					$CACHE_FRONT_END=1;
				} else {
					$CACHE_FRONT_END=0;
				}
			...
			if (!$CACHE_FRONT_END or ($CACHE_FRONT_END and !$content=$Cache_Lite->get($string))) {
				$sql = "select c.custom_settings, c.categories_id, c.parent_id, cd.categories_name, cd.meta_title, cd.meta_description from tx_multishop_categories c, tx_multishop_categories_description cd where c.categories_id = ".$c." and cd.language_id='".$this->sys_language_uid."' and c.categories_id = cd.categories_id";
				$qry = $GLOBALS['TYPO3_DB']->sql_query($sql);
				if ($GLOBALS['TYPO3_DB']->sql_num_rows($qry)) {
					$data = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($qry);
					if ($data['categories_name']) {
						$output[]=array('name' => $data['categories_name'], 'url' => mslib_fe::rewritenamein($data['categories_name'],'cat',$data['categories_id']), 'id' =>$data['categories_id'],'custom_settings' => $data['custom_settings'],'meta_title'=>$data['meta_title'],'meta_description'=>$data['meta_description']);			
					}
					if ($data['parent_id'] > 0 and $data['parent_id'] <> $this->categoriesStartingPoint) {
									...
									case 'radio':
										...
										if (count($sessionData['attributes'][$options['products_options_id']])) {
											foreach ($sessionData['attributes'][$options['products_options_id']] as $item) {
												if ($item['products_options_values_id'] == $products_options_values['products_options_values_id']) $items.=' checked';
											}
										}
	...
	public function getProduct($products_id, $categories_id='', $extra_fields='', $include_disabled_products=0, $skipFlatDatabase=0) {
		...
		if ($skipFlatDatabase or (!$this->ms['MODULES']['FLAT_DATABASE'] or $include_disabled_products)) {
			$select=array();
			$select[]='*';
		...
		$str = $GLOBALS['TYPO3_DB']->SELECTquery(
			(is_array($query_elements['select'])?implode(",",$query_elements['select']):''),         // SELECT ...
...
//		die();
	...
	public function getNextPreviousProduct($products_id, $categories_id='') {
		...
		$str = $GLOBALS['TYPO3_DB']->SELECTquery(
			implode(',',$query_elements['select']),         // SELECT ...
			implode(',',$query_elements['from']),     // FROM ...
			(count($query_elements['filter']) ? implode(' AND ',$query_elements['filter']):'').
			(count($query_elements['where']) ? ' AND '.implode(' AND ',$query_elements['where']):''),    // WHERE...
	...
	public function getProductsPageSet($filter=array(),$offset=0,$limit=0,$orderby=array(),$having=array(),$select=array(),$where=array(),$redirect_if_one_product=0,$extra_from=array(),$groupby=array(),$search_section='products_search',$select_total_count='',$returnTotalCountOnly=0,$enableFetchTaxRate=1,$extra_join=array()) {
			...
			if (!$this->ms['MODULES']['FLAT_DATABASE'])  {
				if (count($having)) {
					...
					$str = $GLOBALS['TYPO3_DB']->SELECTquery(
						'p.products_id,IF(s.status, s.specials_new_products_price, p.products_price) as final_price',         // SELECT ...
						$from_clause,     // FROM ...
						$where_clause,    // WHERE...
	...
	*/	
	...
	public function tx_multishop_get_category_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false, $exclude_not_active = false,$default_label='') {
		...
		if ((sizeof($category_tree_array) < 1) && ($exclude != '0')) {
			$category_tree_array[] = array('id' => $this->categoriesStartingPoint, 'text' => $default_label);
		}
		...
		if ($GLOBALS['TYPO3_DB']->sql_num_rows($qry3)) {
			...
			if ($row3['shipping_costs_type']=='weight') {
				...
				foreach ($steps as $step) {
					$cols=explode(":",$step);
					if (isset($cols[1])) $current_price=$cols[1];
...
*/				
	...
	public function isChecked($pid,$relative_product_id) {
		...
		$query_checking = $GLOBALS['TYPO3_DB']->SELECTquery(
	...
	public function calculateStaffelPrice($staffel_price, $product_qty) {
		...
		foreach ($staffel_data as $keys => $values) {
			$security_max=99999;
			if ($values['max'] > $security_max) $values['max']=$security_max;			
		...
		if ($mode == 'stepping') {
			if ($plevel > 0) {
				...
				foreach ($sdata3 as $keys => $values) {
						if ($qty < $values['count']) $values['count'] = $qty;
...
//				error_log(print_r($data_level,1));
	...
	public function getCMScontent ($type, $language_id=0,$loadFromPids=array()) {
		...
		if (is_array($loadFromPids) and count($loadFromPids)) {
			foreach ($loadFromPids as $loadFromPid) {
				$query = $GLOBALS['TYPO3_DB']->SELECTquery(
					'c.id,cd.name,cd.content,c.hash',         // SELECT ...
	...
	public function getOrderStatusName($id,$language_id=0) {
		...
		$query = $GLOBALS['TYPO3_DB']->SELECTquery('od.name','tx_multishop_orders_status o, tx_multishop_orders_status_description od','o.id=\''.$id.'\' and od.language_id=\''.$language_id.'\' and (o.page_uid=0 or o.page_uid=\''.$this->shop_pid.'\') and o.id=od.orders_status_id','','','');	
		$res = $GLOBALS['TYPO3_DB']->sql_query($query);
		if ($GLOBALS['TYPO3_DB']->sql_num_rows($res) > 0) {
			$row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res);
			return $row['name'];
...
//			$ms_menu['footer']['ms_admin_system']['subs']['admin_shipping_and_payment']['subs']['admin_tax_rules']['link']=mslib_fe::typolink($this->shop_pid,'tx_multishop_pi1[page_section]=admin_tax_rules');		
	...
	public function getOrderInvoice ($orders_id,$create_if_not_exists=1) {
		...
		$qry = $GLOBALS['TYPO3_DB']->sql_query($sql);	
		if ($GLOBALS['TYPO3_DB']->sql_num_rows($qry) == 0) {
			if ($create_if_not_exists) $data=mslib_fe::createOrderInvoice($orders_id,1);
		} else {
			$data = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($qry);
		}
		if($data['invoice_id']) {
	...
	*/
	...
	public function getProductAttributesStockGroupBox ($product) {
		...
		if ($products_count > 0) {
			$content = '';
			if ($products_count) {
				if (!$this->ms['MODULES']['PRODUCTS_ATTRIBUTES_PAIR_TYPE']) $this->ms['MODULES']['PRODUCTS_ATTRIBUTES_PAIR_TYPE']='default';
				if (strstr($this->ms['MODULES']['PRODUCTS_ATTRIBUTES_PAIR_TYPE'],"..")) die('error in PRODUCTS_ATTRIBUTES_PAIR_TYPE value');
				else
				{
					if (strstr($this->ms['MODULES']['PRODUCTS_ATTRIBUTES_PAIR_TYPE'],"/"))	require($this->DOCUMENT_ROOT.$this->ms['MODULES']['PRODUCTS_ATTRIBUTES_PAIR_TYPE'].'.php');
					else	require('includes/products_attributes_pair/'.$this->ms['MODULES']['PRODUCTS_ATTRIBUTES_PAIR_TYPE'].'.php');
				}
			}
		}
		return $content;
	...
	public function taxRuleSet ($tax_group_id, $current_price, $cn_iso_nr = 0, $zn_country_iso_nr = 0) {
		...
		if ($tax_group_id) {
			$sql_local_tax_rate = $GLOBALS['TYPO3_DB']->SELECTquery(
				'mt.rate as tax_rate,mt_c.rate as country_tax_rate,sc.cn_iso_nr as country_id,sc.cn_short_en as country_name,scz.uid as state_id,scz.zn_name_local as state_name,mtr.state_modus',         // SELECT ...
