...
//		$row2=$GLOBALS['TYPO3_DB']->sql_fetch_row($qry2);
	...
	*/
	...
	public function getShippingCosts($countries_id, $shipping_method_id) {
		...
		$str3=$GLOBALS['TYPO3_DB']->SELECTquery('sm.shipping_costs_type, sm.handling_costs, c.price, c.override_shippingcosts, c.zone_id', // SELECT ...
			'tx_multishop_shipping_methods sm left join tx_multishop_shipping_methods_costs c on sm.id=c.shipping_method_id left join tx_multishop_countries_to_zones c2z on c.zone_id=c2z.zone_id and c2z.cn_iso_nr=\''.$countries_id.'\'', // FROM ...
			'sm.id=\''.$shipping_method_id.'\' and (sm.page_uid=0 or sm.page_uid=\''.$this->shop_pid.'\')', // WHERE...
			'', // GROUP BY...
			'', // ORDER BY...
			'' // LIMIT ...
		);
