    ...
    public function rewritenamein($input, $id = '', $replaceDashByUnderscore = 0) {
        ...
        if (is_array($GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['ext/multishop/pi1/classes/class.mslib_fe.php']['rewritenameinPreProc'])) {
            $params = array(
                    'input' => &$input,
                    'id' => &$id,
                    'replaceDashByUnderscore' => &$replaceDashByUnderscore
            );
...
*/
    ...
    public function getSubcatsOnly($parent_id = 0, $include_disabled_categories = 0, $page_uid = '', $include_hidden_categories = 1) {
        ...
        if (is_numeric($parent_id)) {
            $query_array = array();
            $query_array['select'][] = 'c.categories_id,c.hide_in_menu,c.hide_in_footer_menu,cd.categories_name,c.status,c.parent_id,c.categories_image,cd.content,cd.content_footer,cd.shortdescription,cd.categories_external_url,cd.meta_keywords,cd.meta_description';
