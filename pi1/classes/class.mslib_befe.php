...
//			$from[]='tx_multishop_products p left join tx_multishop_specials s on p.products_id = s.products_id left join tx_multishop_manufacturers m on p.manufacturers_id=m.manufacturers_id left join static_taxes tr on p.tax_id = tr.uid';
	...
	public function tep_get_chained_categories_select($categories_id='0', $aid='', $level=0, $selectedid='', $page_uid='') {
		...
		$output=array();
		$str="SELECT cd.categories_name, c.categories_id, c.parent_id from tx_multishop_categories c, tx_multishop_categories_description cd where c.parent_id='".$categories_id."' and c.status=1 and c.categories_id=cd.categories_id and c.page_uid='".$page_uid."' order by c.sort_order, cd.categories_name";
...
//							$result .= t3lib_utility_Debug::viewArray($val);
	...
	public function RunMultishopUpdate($multishop_page_uid='') {
		...
		if (!count($messages)) {
			$content='Nothing updated. You are already using the latest version.';
		} else {
	...
	/*
		public function updateOrderStatus($orders_id,$orders_status_id,$customer_notified=0)
		{
